...
describe('ResponseMeta', () => {
  it('renders the status code via StatusBadge', () => {
    render(<ResponseMeta status={200} statusText="OK" durationMs={100} sizeBytes={512} />);
  ...
  it('renders formatted duration for 150ms', () => {
    render(<ResponseMeta status={200} statusText="OK" durationMs={150} sizeBytes={512} />);
    expect(screen.getByText('150ms')).toBeInTheDocument();
  });

  it('renders formatted duration for 2000ms → 2.0s', () => {
    render(<ResponseMeta status={200} statusText="OK" durationMs={2000} sizeBytes={512} />);
  ...
  it('renders formatted size for 1024 bytes → "1.0 KB"', () => {
    render(<ResponseMeta status={200} statusText="OK" durationMs={100} sizeBytes={1024} />);
    expect(screen.getByText('1.0 KB')).toBeInTheDocument();
  });

  it('renders Clock icon (svg element present)', () => {
    const { container } = render(
      <ResponseMeta status={200} statusText="OK" durationMs={100} sizeBytes={512} />,
    );
