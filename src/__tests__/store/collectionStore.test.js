...
const makeSavedRequest = (overrides = {}) => ({
    id: 'req-1',
    name: 'Get Users',
    request: makeRequest(),
    ...overrides,
...
describe('useCollectionStore — fetchCollections()', () => {
    ...
    it('sets collections in state from tauri response', async () => {
        const cols = [makeCollection({ id: 'col-a', name: 'Alpha' })];
    ...
    it('sets isLoading true then false around the fetch', async () => {
        const loading = [];
        let resolveGetCollections;
        const deferred = new Promise((res) => { resolveGetCollections = res; });
