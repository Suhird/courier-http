...
import type { RequestConfig, HttpResponse, Environment } from '../../types';
...
describe('useEnvironmentStore — fetchEnvironments()', () => {
  it('sets isLoading true then false around the fetch', async () => {
    ...
    let resolveGetEnvironments!: (v: Environment[]) => void;
    const deferred = new Promise<Environment[]>((res) => { resolveGetEnvironments = res; });
  ...
  it('loads environments returned by tauri.getEnvironments', async () => {
    const envs: Environment[] = [makeEnvironment({ id: 'env-a', name: 'Alpha' })];
